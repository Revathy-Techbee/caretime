(function ($, wijmo, helper, props) {
    ...
    menu.selectedIndexChanged.addHandler(function (sender) {
        ...
        var direction = sender.selectedValue,
            $dirCols = $('.direction-col');

        // split or stack columns
        $dirCols.each(function () {
        ...
        if (direction === 'Up' || direction === 'Down') {
            $(linearGauge.hostElement).addClass('vertical-gauge');
            $(bulletGraph.hostElement).addClass('vertical-gauge');
        }
        else {
            $(linearGauge.hostElement).removeClass('vertical-gauge');
            $(bulletGraph.hostElement).removeClass('vertical-gauge');
        }
