(function () {
    ...
    var cvGrouping = new wijmo.collections.CollectionView(getData(20)),
        ...
        groupingFieldNameList = $('#groupingFieldNameList'),
    ...
    groupingFieldNameList.append('<option value="" selected="selected">Please choose the field you want to group by...</option>');
    for (var i = 0; i < groupingNames.length; i++) {
        groupingFieldNameList.append('<option value="' + groupingNames[i] + '">' + groupingNames[i] + '</option>');
    }
    groupingFieldNameList.on('change', groupGrid);

    // update the group settings.
    function groupGrid() {
        var gd,
            fieldName = groupingFieldNameList.val();
