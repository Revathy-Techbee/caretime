module wijmo.grid {
	...
	export class ExcelConverter {
		 ...
		 */
		static export(flex: any, convertOption: IConverterOption = { includeColumnHeader: true }): IExcelConvertResult {
			var file = {
				worksheets: [],
				creator: 'Mike Lu',
				created: new Date(),
		 ...
		 */
		static import(file: ArrayBuffer, flex: FlexGrid, convertOption: IConverterOption = { includeColumnHeader: true }, moreSheets?: FlexGrid[]) {
			var fileData = this._base64EncArr(new Uint8Array(file)),
				fileObj = xlsx(fileData),
				currentIncludeRowHeader = convertOption.includeColumnHeader,
				sheetCount = 1,
				sheetIndex = 0,
				c = 0,
				r = 0,
				...
				for (; r < currentSheet.data.length; r++) {
					...
					if (row && row[0]) {
						...
						while (nextRowIdx < currentSheet.data.length) {
							...
							if (nextRow) {
								if ((isNaN(row[0].groupLevel) && !isNaN(nextRow[0].groupLevel))
									|| (!isNaN(row[0].groupLevel) && row[0].groupLevel < nextRow[0].groupLevel)) {
									isGroupHeader = true;
								}
								break;
							} else {
		...
		private static _exportFlexGrid(flex: FlexGrid, file: any, convertOption: IConverterOption) {
			var worksheetData: any[] = [],
				columnSettings: any[] = [],
		...
		private static _parseFlexGridRowToSheetRow(flex: FlexGrid, row: Row, rowIndex: number, columnSettings: any[], isGroupRow: boolean, groupLevel: number): any[] {
			...
			for (var ci = 0; ci < flex.columnHeaders.columns.length; ci++) {
				...
				isFormula = val && isString(val) && val.length > 1 && val[0] === '=';
				format = columnSetting.format ? this._parseCellFormat(columnSetting.format) : isDate(unformattedVal) ?
				this._formatMap['d'] : !isNumber(unformattedVal) || isInt(unformattedVal) ?
				'General' : this._formatMap['n'];
				...
				if (isGroupRow && row['hasChildren'] && ci === flex.columns.firstVisibleIndex) {
					...
					worksheetDataItem.push({
						...
						formatCode: format,
						autoWidth: true,
						hAlign: isDate(unformattedVal) && columnSetting.alignment === '' ? 'left' : columnSetting.alignment,
						width: columnSetting.width,
						height: rowHeight,
						visible: columnSetting.visible,
		...
		private static _parseCellFormat(format): string {
			var dec = 0,
				mapFormat = this._formatMap[format[0]],
				decimalArray = [],
				xlsxFormat: string;
		...
		private static _parseExcelFormat(item: any): string {
		...
		private static _parseExcelFormula(formula: string): string {
			var func = formula.substring(1, formula.indexOf('(')),
				format: string;
		...
		private static _getColumnSetting(column: Column, defaultWidth: number): any {
		...
		private static _setColumn(columns: IColumn[], columnIndex: number, item: any) {
			...
			if (!columns[columnIndex]) {
				...
				dataType = this._getItemType(item);
				if (columns[columnIndex].dataType === undefined || (dataType !== undefined && dataType !== wijmo.DataType.String && columns[columnIndex].dataType === wijmo.DataType.String)) {
		...
		private static _base64DecToArr(sBase64: string, nBlocksSize?: number): Uint8Array {
		...
		private static _base64EncArr(aBytes: Uint8Array): string {
			var nMod3 = 2, sB64Enc = "";
...
/**
 * Excel Convert option
 */
interface IConverterOption {
	/**
	 * indicates whether include column header for exporting and importing.
	 */
	includeColumnHeader: boolean;
	// Todo add other convert option
}
...
/**
 * Excel Convert Result for exproting flex grid to xlsx
 */
interface IExcelConvertResult {
	/**
