        ...
        var ODataCollectionView = (function (_super) {
                ...
                if (options) {
                    ...
                    if (options.serverSettings) {
                        ...
                        if (options.serverSettings.take) {
                            this._pgSz = options.serverSettings.take;

                            if (options.serverSettings.skip) {
                                this._pgIdx = Math.floor(options.serverSettings.skip / options.serverSettings.take);
                            }
                        }
                    }
                }
            ...
            */
            ...
            ODataCollectionView.prototype._querySuccess = function (res, isContinued) {
                var resData = [], self = this, nextLink = res['odata.nextLink'], needRootUrl = true;
                ...
                if (res.value) {
                    ...
                    } else if (res.d.EntitySets) {
                        resData = res.d.EntitySets;
                    }
                    if (!nextLink) {
                        nextLink = res.d['__next'];
                        needRootUrl = false;
                    }
                } else if ($.isArray(res)) {
                    resData = res;
                ...
                if (this._loadAllData && nextLink) {
                    this._requestData(needRootUrl ? this._serviceUrl + nextLink : nextLink, {
                        data: {
                            $format: 'verbosejson'
                        }
                    }, true);
