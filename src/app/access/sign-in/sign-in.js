            ...
            $scope.signInUser = function(user) {
                ...
                if ($scope.singInForm.$valid) {
                    Services.signinService.get({
                        filter: "(user_email='" + $scope.user.user_email + "' and user_password = '" + $scope.user.user_password + "' and status > 0) or (user_name='" + $scope.user.user_email + "' and user_password = '" + $scope.user.user_password + "' and status > 0)"
                    }, function(data) {
                        if (data.record.length >= 1) { //authenticated user
                            ...
                            var login_agency_id=data.record[0]['agency_id'];
                                ...
                                mixpanel.people.set({
                                    "$email": $scope.user.user_email, // only special properties need the $
                                ...
                                Services.employeeService.get({
                                    filter: "id=" + user_info.employee_id
                                }, function(data) {
                                ...
                                if(login_agency_id)
                                {
                                    ...
                                    Services.agencyDetail.update({
                                        id: login_agency_id
                                    }, $scope.agencyDBField, function(updatedata) {
                                    });
                                }
                            /* $scope.show_form_loader = false;
                        ...
                        } else { //unathenticated user
                            var error_msg = "";
                            Services.signinService.get({
                                filter: "(user_email='" + $scope.user.user_email + "') or (user_name='" + $scope.user.user_email + "')"
                            }, function(data) {
