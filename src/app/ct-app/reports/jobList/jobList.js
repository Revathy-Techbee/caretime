...
.config(['$stateProvider', function config($stateProvider) {
    ...
    $stateProvider.state('ctApp.jobList', {
        url: '/jobList',
        views: {
            "appNested": {
                controller: 'JobListCtrl',
                templateUrl: 'ct-app/reports/jobList/jobList.tpl.html'
            }
        },
        data: {
            pageTitle: 'Schedule Reporting',
            access: access.jobList
        }
    });

}])

.controller("JobListCtrl", ["$scope", "Services", "HelperService", "$timeout","$localStorage", function($scope, Services, HelperService, $timeout,$localStorage) {
    $scope.reportFilters = {};
    ...
    $scope.loadData = function(fdate, ldate, zone, fieldname,  offset) {
        var filterObj;
         filterObj = {
                'fields': "job_code,job_name,job_address1,job_city,job_state,job_zip,authorized_phone_format,status,last_clocked_in_date,last_scheduled_date,created_on",
        ...
        Services.jobService.get(filterObj, function(data) {
            angular.forEach(data.record, function(item, key) {
                //tempzone = JSON.parse(item.zone_detail);
                $scope.resultData.push({
                        "job_code": (item.job_code)?(item.job_code).toString():"",
                        "job_name": item.job_name,
                        "job_address1": item.job_address1,
                        "job_city": item.job_city,
                        "job_state": item.job_state,
                        "job_zip": item.job_zip,
                        "authorized_phone_format": item.authorized_phone_format,
    ...
    $scope.updateTableData = function(isFilter) { // on limit change
       ...
       mixpanelObj={
            ...
            mixpanel.track("Job List Reporting",mixpanelObj);
        $scope.loadData(fdate, ldate, $scope.reportFilters.zone, $scope.reportFilters.field, 0);


    };
    $scope.noRecord = 1;
    $scope.norecord = HelperService.errorMsg('alert-danger', 'Please Search to get Job List');
