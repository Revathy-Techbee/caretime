angular.module('ctApp.activityCodes', [
    'ui.router'
])
...
.controller("ActivityCodesCtrl", ["$scope", "Services", "$state", "$modal", "$localStorage",
    function($scope, Services, $state, $modal, $localStorage) {
        ...
        $scope.enableEditView = function(id) {
            $state.go("ctApp.addUpdateActivityCode", {
                activityCodeId: id
            });
        };
...
.controller("AddUpdateActivityCodeCtrl", ["$scope", "Services", "$state", "$stateParams", "$timeout", "HelperService", "$localStorage",
        ...
        $scope.getActivityCodeDetail = function() {
            if ($scope.activityCode_id) {
                ...
                Services.activity_code.get({
                    "filter": "id='" + $scope.activityCode_id + "'"
                }, function(resp) {
...
]).controller("DeleteActivityCodeCtrl", ["$scope", "Services", "$modalInstance", function($scope, Services, $modalInstance) {
