...
.controller("SkillsCtrl", ["$scope", "Services", "$state", "$modal", "$localStorage",
    function($scope, Services, $state, $modal, $localStorage) {
        ...
        $scope.enableDelete = function(id) {
            ...
            $scope.modalInstance.result.then(function(id) {
                Services.auth_skillsService.delete({
                    filter: "id='" + id + "'"
                }, function(data) {
                    $scope.logger = {};
                    ...
                    $scope.logger.action_table ="auth_skills";
...
.controller("AddUpdateSkillCtrl", ["$scope", "Services", "$state", "$stateParams", "$timeout", "HelperService", "$localStorage",
        function($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage) {
            ...
            $scope.getSkillDetail = function() {
                if ($scope.skill_id) {
                    ...
                    Services.auth_skillsService.get({
                        "filter": "id='" + $scope.skill_id + "'"
                    }, function(resp) {
                ...
                if ($scope.addUpdateSkillForm.$valid) {
                    ...
                    Services.auth_skillsService.get($scope.filterObj, function(data) {
                        if (data.record.length > 0) {
                            ...
                            if ($scope.skill_id) { // means it is in edit state
                            ...
                            if ($scope.skill_id) {
                                ...
                                Services.auth_skillsService.update({
                                    id: $stateParams.skillId
                                }, $scope.skillDBField, function(data) {
                                    $scope.logger = {};
                                    $scope.logger.userid = $localStorage.user_info.user_id;
                                    $scope.logger.user_detail = JSON.stringify({
                                        "username": $localStorage.user_info.username,
    ...
    .controller("DeleteSkillCtrl", ["$scope", "Services", "$modalInstance", function($scope, Services, $modalInstance) {
