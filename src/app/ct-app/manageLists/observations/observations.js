angular.module('ctApp.observations', [
    'ui.router'
])
...
.controller("ObservationsCtrl", ["$scope", "Services", "$state", "$modal",
    function($scope, Services, $state, $modal) {
        ...
        $scope.enableEditView = function(id) {
            $state.go("ctApp.addUpdateObservation", {
                observationId: id
            });
        };
        $scope.enableDelete = function(id) {
            ...
            $scope.modalInstance.result.then(function(id) {
                Services.observationsName.delete({
                    filter: "id='" + id + "'"
                }, function(data) {
                    $scope.updateTableData();
                });
...
.controller("AddUpdateObservationCtrl", ["$scope", "Services", "$state", "$stateParams", "$timeout", "HelperService", "$localStorage",
    function($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage) {
        ...
        $scope.getObservationDetail = function() {
            if ($scope.observation_id) {
                ...
                Services.observationsName.get({
                    "filter": "id='" + $scope.observation_id + "'"
                }, function(resp) {
                    ...
                    angular.extend($scope.observation, {
                        ...
                        // created_by: $scope.observationDBField.created_by,
                        editedOn: HelperService.converttimeZone($scope.observationDBField.modified_date),
                        // edited_by: $scope.observationDBField.edited_by
                    });
                });
...
]).controller("DeleteObservationCtrl", ["$scope", "Services", "$modalInstance", function($scope, Services, $modalInstance) {
