...
.controller("SchedulesCtrl", ["$scope", "Services", "$state", "$modal", "$timeout", "HelperService", "$localStorage", function($scope, Services, $state, $modal, $timeout, HelperService, $localStorage) {
    $scope.config = {
        general: {
            searchtxt: ""
        },
...
.controller("AddUpdateScheduleCtrl", ["$scope", "Services", "$state", "$stateParams", "$timeout", "HelperService", "$localStorage", "$modal", "$sce", "$window","$rootScope",
    function($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage, $modal, $sce, $window,$rootScope) {
        ...
        $scope.shift.recurrence = {}; // no recure
        $scope.shift.recurrence.is = 0;
        $scope.shift.recurrence.type = 0; // defailt daily 
        ...
        if ($scope.shiftId || $scope.eventcode) {
            ...
            if ($scope.allshiftId) {
                ...
                filterObj = {
                    filter: "id='" + $scope.shiftId + "' and status > 0"
                };
                ...
                if ($scope.shiftDBField.recurrence) {
                    ...
                    if (angular.isDefined($scope.shiftDBField.recurrence.end)) {
                        ...
                        if (angular.isDefined($scope.shiftDBField.recurrence.end.date)) {
                            ...
                            $scope.shift.recurrence.end.date = ($scope.shiftDBField.recurrence.end.date) ? moment($scope.shiftDBField.recurrence.end.date).toDate() : "";
                            /* $scope.shift.recurrence.end.date =($scope.shiftDBField.recurrence.end.date) ? moment($scope.shiftDBField.recurrence.end.date).format("YYYY-MM-DD") : "";*/
        ...
        $scope.getRecursiveDate = function(recuriveType) {
            ...
            if (recuriveType == "0") { // daily 

                if ($scope.shift.recurrence.daily.isdayLimit == "1") { // if it is 1 daytime if 2 the it every week day
                    ...
                    $scope.recObj = momementObj.recur().every([1, 2, 3, 4, 5]).daysOfWeek(); // setrecuriveevery week day
                }

                if ($scope.shift.recurrence.end.isendOccur == "1") { // if end by number occurence 

                    if (moment(momementObj).isBefore(moment())) { // check in at date in after today
                ...
                } else if ($scope.shift.recurrence.end.isendOccur == "2") { // if end date is seleceted 
                    if (moment(momementObj).isBefore(moment())) { // check in at date in after today
            ...
            } else if (recuriveType == "1") { //weekly



                if ($scope.shift.recurrence.end.isendOccur == "1") { // if end by number occurence

                    $scope.recObj = momementObj.recur().every($scope.shift.recurrence.week.weeklimt).weeks(); // setrecurive
                    ...
                    for (i = 0; i < weekDates.length; i++) {
                        ...
                        if (recurDates.length < $scope.shift.recurrence.end.occurcount) {
                            ...
                            if (i === 0) {
                                ...
                                weekDayStart = moment(tempMomt).add(1, 'days').format("MM/DD/YYYY"); // add 1 day bacase 1 days consider  to add or not on above
                                endDate = moment(moment(weekDayStart).add(5 - Number(tempMomt.format("d")), 'days'));
                                // 5 because 1 day already added
                            ...
                            if (($scope.shift.recurrence.end.occurcount - recurDates.length) >= weekDaysArray.length) {
                                ...
                                temprec.endDate(moment(endDate).format("MM/DD/YYYY")); // set end sate as next week start

                                if (moment(weekDayStart).isBefore(moment(endDate))) { // check if end date is greater then start date
                                ...
                                temprec.startDate(startDate.format("MM/DD/YYYY")); // set start date before 1 day ago, as next function skips day start
                    ...
                    $scope.recObj = WeekStartDateObj.recur({
                        "end": moment($scope.shift.recurrence.end.date).format("MM/DD/YYYY")
                    }).every($scope.shift.recurrence.week.weeklimt).weeks(); // setrecurive
                    ...
                    for (i = 0; i < weekDates.length; i++) {
                        ...
                        if (moment(weekDates[i]).isBefore(moment($scope.shift.recurrence.end.date).format("MM/DD/YYYY"))) { // end date cant be greater then start date
                            // start date  starts
                            if (i === 0) {
                            ...
                            if (i === 0) {
                                ...
                                if (moment(endDateRecEnd).isBefore(moment(endDate).format("MM/DD/YYYY"))) { // end date cant be greater then actual end date
                            ...
                            $scope.recObj.endDate(endDate); // set end sate as next week start
            ...
            } else if (recuriveType == "2") { // monthy

                if ($scope.shift.recurrence.end.isendOccur == "1") { // if number occur 

                    if ($scope.shift.recurrence.month.isDay == "1") { //opt 1

                        recObj = momementObj.recur().every($scope.shift.recurrence.month.thMonth).months(); // setrecurive

                        monthDates = recObj.next($scope.shift.recurrence.end.occurcount, "L");

                        if (moment(momementObj.date($scope.shift.recurrence.month.thDay)).isBefore(moment())) { // check in at date in after today
                    ...
                    } else if ($scope.shift.recurrence.month.isDay == "2") { //opt2 not working now

                        recObj = momementObj.recur().every($scope.shift.recurrence.month.thEMonth).months(); // setrecurive
                ...
                } else if ($scope.shift.recurrence.end.isendOccur == "2") { // if end date choosen


                    recObj = momementObj.recur({
                        "end": moment($scope.shift.recurrence.end.date).format("MM/DD/YYYY")
                    }).every($scope.shift.recurrence.month.thMonth).months(); // setrecurive
                    monthDates = recObj.all("L");
                    recurDates = [];
                    for (i = 0; i < monthDates.length; i++) {

                        if (moment(moment(monthDates[i]).date($scope.shift.recurrence.month.thDay).format("MM/DD/YYYY")).isBefore(moment($scope.shift.recurrence.end.date).format("MM/DD/YYYY"))) { //month date end date cant be greater then start date
        ...
        $scope.scheduleManage = function(step) {
            ...
            if (step == "recurrence") {
                ...
                if (moment($scope.shift.inAt).isBefore(moment())) { // check in at date in before today day (if it is past date)
                    $scope.savedisable = 0;
                    $scope.showerrorMsg = true;
                    $scope.ErrorClass = "danger";
                ...
                if ($scope.shift.recurrence.end.isendOccur == 2 && $scope.shift.recurrence.is == '1') {
                    ...
                    if (moment($scope.shift.inAt).isAfter(moment($scope.shift.recurrence.end.date))) { // check in at date in after end day
                        $scope.savedisable = 0;
                        $scope.showerrorMsg = true;
                        $scope.ErrorClass = "danger";
                    ...
                    if ($scope.shift.recurrence.type == 1) {
                        ...
                        if (moment(moment($scope.shift.inAt).add(6, "days")).isAfter(moment($scope.shift.recurrence.end.date))) { // check in at date in after end day
                            $scope.savedisable = 0;
                            $scope.showerrorMsg = true;
                            $scope.ErrorClass = "danger";
                ...
                $scope.shiftDBField[0].ref_in_at = moment($scope.shift.inAt).utc().format('YYYY-MM-DD HH:mm'); // newly added
                $scope.shiftDBField[0].ref_out_at = moment($scope.shift.outAt).utc().format('YYYY-MM-DD HH:mm'); // newly added
                    ...
                    angular.forEach($scope.tempoutputDates, function(item, key) {
                        ...
                        if (keepGoing) {
                            ...
                            if (!$scope.WarningClass) {
                                ...
                                $scope.shiftDBField[key].in_time = inAtCurrent.utc().format("HH:mm:ss A"); // in time will be same in all days regardless of date
                                $scope.shiftDBField[key].in_at = inAtCurrent.utc();
                                $scope.shiftDBField[key].out_date = outAtCurrent.utc().format("MM/DD/YYYY"); //$scope.getOutDate(item,$scope.shift.duration) ;
                                $scope.shiftDBField[key].out_time = outAtCurrent.utc().format("HH:mm:ss A"); // in time will be same in all days regardless of date
                                $scope.shiftDBField[key].out_at = outAtCurrent.utc();
                                $scope.shiftDBField[key].ref_in_at = inAtCurrent.utc().format('YYYY-MM-DD HH:mm'); // newly added
                                $scope.shiftDBField[key].ref_out_at = outAtCurrent.utc().format('YYYY-MM-DD HH:mm'); // newly added
        ...
        $scope.removeShifts = function() {
            ...
            if (($scope.shiftId) && $scope.allshiftId === "") {
                ...
                $scope.deleteCode = {
                    filter: "shift_code='" + $scope.eventcode + "' and agency_id = " + Services.getAgencyID()
                };
...
.controller("recurrenceShiftCtrl", ["$scope", "Services", "$modalInstance", function($scope, Services, $modalInstance) {
...
}]).controller("DeleteShiftCtrl", ["$scope", "Services", "$modalInstance", function($scope, Services, $modalInstance) {
