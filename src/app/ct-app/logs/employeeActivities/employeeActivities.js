...
.controller("EmployeeActivitiesCtrl", ["$scope", "Services", "HelperService", "$timeout", "$localStorage",
            ...
            Services.employeeActivitiesService.get(filterObj, function(data) {
                angular.forEach(data.record, function(item, key) {
                    $scope.employeeActivityList.push({
                        ...
                        "orig_clock_in": HelperService.formatingDate(item.orig_clock_in),
                        "orig_clock_out": HelperService.formatingDate(item.orig_clock_out),
