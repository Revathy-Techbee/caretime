...
angular.module('utils.ct.filters', [])
    .filter('fromNow', function() {
        return function(date) {
            return moment(date).fromNow();
        };
    })
    .filter('parseToJson', function($sce) {
    ...
    .filter('checkstatus', function($sce) {
     ...
     .filter('checkAgencystatus', function($sce) {
        return function(data,fieldToShow) {
            if(data=="1"){
                return  $sce.trustAsHtml('<span class="label bg-success">Active</span>');
            }else if(data=="2"){
    ...
    .filter('checkStatStatus', function($sce) {
        return function(data,fieldToShow) {
            if(data=="1"){
    ...
    .filter('datecustomeFormat', function() {
        return function(data) {
            return data ;
            /*if(data && angular.isUndefined(date.split())){
                //console.log(data);
                var temp = data.split("-");
    ...
    .filter('checklevel', function($sce) {
        return function(data,fieldToShow) {

            var arr=['Nursing','Admin','HR','Office Clerical','Office Manager','On Call','Corporate Billing Department','Scheduler','Basic'];
             return  $sce.trustAsHtml(arr[data]);
           
        };
    })
    .filter('phoneFormat', function($sce) {
        return function(data,fieldToShow) {
            if(data){
    ...
    .filter('job_phone_list', function($sce) {
        return function(data,fieldToShow) {
            if(data){
    ...
    .filter('billable_hrs', function($sce) {
        return function(data,fieldToShow) {
            var temp = null;
            if(data ===1){
    ...
    .filter('duration_in_decimal', function($sce) {
        return function(data,fieldToShow) {
            var temp = null;
            if(data){
...
.filter('removeJSONCreateName', function() {
  ...
  .filter('SecPhoneParseToJson', function($sce) {
    return function(data,fieldToShow,ifMe) {
          if(data){
          var temp = JSON.parse(data);
              var tempar = "";
...
.filter('calenderIconColor', function($sce) {
   return function(date) {
       if (date) {
           dateAndTime = moment.tz(date, "UTC").format();
           sdate = moment(dateAndTime).format("MM/DD/YYYY");
