...
app.controller('CustomerCtrl', ['$scope', function ($scope) {
	...
	$scope.customers = new wijmo.data.BreezeCollectionView(app.dataservice.getManager(), app.dataservice.getEntityQuery("Customers"), true, true);
	$scope.customer = null;
    ...
    $scope.$watch('filterText', function (newVal, oldVal) {
        if (newVal !== oldVal) {
            $scope.customers.filterPredict = app.dataservice.getPredict(newVal);
        }
    });
}]);
...
app.controller('OrderCtrl', function ($scope) {
	...
	$scope.orders = new wijmo.data.BreezeCollectionView(app.dataservice.getManager(), app.dataservice.getEntityQuery("Orders").take(50), true, true);
