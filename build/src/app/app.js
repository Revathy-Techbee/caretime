...
angular.module('app', [
  ...
  function run($rootScope, $cookies, $state, $http, Auth, Services) {
    ...
    $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
      if (!Auth.authorize(toState.data.access)) {
        //if not authorize to access the route
        ...
        if (fromState.url === '^') {
          if (Auth.isLoggedIn()) {
            ...
            $state.go('access.signin', { agencyService: Services.serviceName });
          }
        }
      } else {
        if (Auth.isLoggedIn()) {
  ...
  function AppCtrl($scope, $rootScope, $translate, $localStorage, $window, Services, Auth, $state) {
    ...
    $localStorage.timezone = 'America/New_York';
    // by muni need to be dynamic
  ...
  function ($resource, $http, $location) {
    ...
    //var baseurl = "https://caretimevisit.com/";
    ...
    var time = new Date();
    var globaltime = '';
    //var appUrl = "https://caretimevisit.com/caretime/index.html";
    ...
    return {
      ...
      getTimeZones: function getTimeZones(val) {
        return $http.get(baseurl + '/rest/' + serviceName + '/agency_zip/?app_name=' + appName + '&fields=timezone&filter=timezone like "%' + val + '%" and status>0 and agency_id=' + globalAgency + ' group by timezone&limit=5', { params: {} });
      },
