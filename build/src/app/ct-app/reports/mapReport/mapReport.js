angular.module('ctApp.mapReport', [
  ...
  '$timeout',
  '$localStorage',
  function ($scope, $rootScope, Services, HelperService, $timeout, $localStorage) {
    $scope.sortType = 'asc';
    $scope.sortme = false;
    $scope.reportFilters = {};
    ...
    $scope.callMap = function () {
      ...
      $scope.ctmap = L.mapbox.map('map', 'rajivperera.jo4glk0p', { zoomControl: true });
      $scope.ctmap.setView([
        $scope.ctmapBaseLat,
        $scope.ctmapBaseLon
      ], 8);
      ...
      $scope.directionsLayer = L.mapbox.directions.layer($scope.directions).addTo($scope.ctmap);
      $scope.directionsLayer.originMarker = L.marker([
        0,
        0
      ], {
        draggable: false,
      ...
      $scope.directionsLayer.destinationMarker = L.marker([
        0,
        0
      ], {
        draggable: false,
      ...
      var directionsInputControl = L.mapbox.directions.inputControl('inputs', $scope.directions).addTo($scope.ctmap);
      ...
      var directionsErrorsControl = L.mapbox.directions.errorsControl('errors', $scope.directions).addTo($scope.ctmap);
      var directionsRoutesControl = L.mapbox.directions.routesControl('routes', $scope.directions).addTo($scope.ctmap);
      var directionsInstructionsControl = L.mapbox.directions.instructionsControl('instructions', $scope.directions).addTo($scope.ctmap);
      var test = [];
    ...
    $scope.format = function (waypoint) {
      if (!waypoint) {
        ...
        return waypoint.geometry.coordinates[0].toFixed(precision) + ', ' + waypoint.geometry.coordinates[1].toFixed(precision);
      } else {
