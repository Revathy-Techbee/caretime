angular.module('ctApp.employeeTimecard', [
  ...
  function ($scope, Services, HelperService, $timeout, $localStorage) {
      ...
      Services.employeeActivitiesService.get(filterObj, function (data) {
        ...
        if (data.meta.count > offset + $scope.call_limit) {
          ...
          if ($scope.resultData.length !== 0) {
            ...
            $scope.EmployeeDetails.groupDescriptions.clear();
    ...
    $scope.clearSearch = function () {
      ...
      $scope.noRecord = 1;
      $scope.showRecord = 0;
      $scope.norecord = HelperService.errorMsg('alert-danger', 'Please Search to get Timecard Report by Employee');  //$scope.updateTableData();
    };
    $scope.downloadpdf = function () {
      ...
      angular.forEach($scope.resultData, function (value, key) {
        if (angular.isUndefined(resultarray[value.employee_code])) {
          // to create header for each employee
        ...
        if (value.work_duration_rounded) {
          roundtime_float = value.work_duration_non_rounded_number.toString();
          //HelperService.timeToFloat(value.work_duration_non_rounded_number);
