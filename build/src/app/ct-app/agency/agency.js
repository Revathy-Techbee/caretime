...
*/
angular.module('ctApp.agency', ['ui.router']).config([
  '$stateProvider',
  ...
  function ($scope, Services, $state, $modal, HelperService, $stateParams, $localStorage, $timeout, $rootScope) {
    ...
    if (!angular.isUndefined($stateParams.agencyId) && $stateParams.agencyId) {
      Services.agencyDetail.get({ id: $stateParams.agencyId }, function (data) {
        $scope.agency = {};
    ...
    $scope.agencyManage = function (step) {
      if (step == 'general') {
        ...
        $scope.empSteps.configuration = true;  //$scope.empSteps.payclass = true;
      }
      if (step == 'configuration') {
        ...
        Services.agencyDetail.get({
          ...
          if (data.record.length < 1) {
            ...
            if (!angular.isUndefined($stateParams.agencyId) && $stateParams.agencyId) {
              // means it is in edit state 
              Services.agencyDetail.update({ id: $stateParams.agencyId }, $scope.agencyDBField, function (data) {
                ...
                Services.log_details.save({ dbname: 'log_agency' }, $scope.logger, function (data) {
                });
                $scope.showerrorMsg = true;
                $scope.ErrorClass = 'success';
            ...
            $timeout(function () {
              $scope.showerrorMsg = false;  //$state.go("app.employees");
            }, 3000);
            return false;
          }
        });
      ...
      if (newValue) {
        ...
        if ($scope.mymapVariable.address_components) {
          ...
          if ($scope.mymapVariable.geometry) {
            $scope.agency.long_lat = {};
            $scope.agency.long_lat.long = $scope.mymapVariable.geometry.location['D'];
            $scope.agency.long_lat.lat = $scope.mymapVariable.geometry.location['k'];
