angular.module('ctApp.zones', ['ui.router']).config([
  '$stateProvider',
  ...
  function ($scope, Auth, Services, $window, $state, $modal, HelperService, $localStorage) {
    ...
    $scope.getNextData = function (offset) {
      ...
      var filterObj = {
          ...
          include_count: true,
          offset: $scope.zoneDetailList.length,
    ...
    $scope.enableEditView = function (id) {
      $state.go('ctApp.addUpdateZone', { zoneId: id });
    };
    $scope.enableEdit = function (id) {
      $state.go('ctApp.zonestats', { statsId: id });
    };
  }
]).controller('AddUpdateZoneCtrl', [
  '$scope',
  ...
  function AddUpdateZoneCtrl($scope, Services, $state, $stateParams, $timeout, $localStorage, HelperService, $window) {
    ...
    $scope.savedisable = 0;
    /* Edited by Lavanya */
    $scope.zone.notify_empname = [{ 'Employee': '' }];
    ...
    $scope.addEditZone = function () {
      if ($scope.zone_id) {
        // means it is in edit state                     
        Services.employeeZones.update({ id: $stateParams.zoneId }, $scope.zoneDBField, function (data) {
          $scope.show_zone_form_loader = false;
        ...
        $scope.getZonecode();
        Services.employeeZones.get({ filter: 'zone_code=\'' + $scope.zoneDBField.zone_code + '\'' }, function (data) {
          if (data.record.length >= 1) {
            ...
            Services.employeeZones.save($scope.zoneDBField, function (data) {
              ...
              Services.employeeZones.update({ id: data.id }, $scope.zoneDBField, function (data) {
    ...
    var showMessageFunc = function (error_msg, error_class, callback) {
      ...
      if (callback) {
        ...
        $timeout(function () {
          $scope.showerrorMsg = false;  ///$state.go("app.zones");
        }, 3000);
      }
      $scope.showerrorMsg = true;
    };
    ...
    $scope.addEmpdetails = function (index) {
      ...
      $scope.zone.notify_empname.push({ 'Empoyee': '' });
    };
