angular.module('ctApp.activities', ['ui.router']).config([
  '$stateProvider',
  ...
  function ($scope, Services, $state, $modal, $localStorage) {
    ...
    $scope.enableEditView = function (id) {
      $state.go('ctApp.addUpdateActivity', { activityId: id });
    };
    $scope.enableDelete = function (id) {
      ...
      $scope.modalInstance.result.then(function (id) {
        Services.auth_activityService.delete({ filter: 'id=\'' + id + '\'' }, function (data) {
          $scope.updateTableData();
        });
      }, function () {
      });
  ...
  function ($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage) {
    ...
    $scope.getActivityDetail = function () {
      if ($scope.activity_id) {
        ...
        Services.auth_activityService.get({ 'filter': 'id=\'' + $scope.activity_id + '\'' }, function (resp) {
    ...
    $scope.activityManage = function (step) {
      ...
      if ($scope.addUpdateActivityForm.$valid) {
        ...
        if ($scope.activity_id) {
          Services.auth_activityService.update({ id: $stateParams.activityId }, $scope.activityDBField, function (data) {
            $scope.show_activity_form_loader = false;
