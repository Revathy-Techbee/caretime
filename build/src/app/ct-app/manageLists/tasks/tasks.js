angular.module('ctApp.tasks', ['ui.router']).config([
  '$stateProvider',
  ...
  function ($scope, Services, $state, $modal, $localStorage) {
    ...
    $scope.enableEditView = function (id) {
      $state.go('ctApp.addUpdateTask', { taskId: id });
    };
    $scope.enableDelete = function (id) {
      ...
      $scope.modalInstance.result.then(function (id) {
        Services.tasksName.delete({ filter: 'id=\'' + id + '\'' }, function (data) {
          $scope.logger = {};
          ...
          $scope.logger.action_table = 'tasks2';
  ...
  function ($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage) {
    ...
    $scope.getTaskDetail = function () {
      if ($scope.task_id) {
        ...
        Services.tasksName.get({ 'filter': 'id=\'' + $scope.task_id + '\'' }, function (resp) {
        ...
        Services.tasksName.get($scope.filterObj, function (data) {
          if (data.record.length > 0) {
            ...
            if ($scope.task_id) {
              ...
              $scope.taskDBField.modified_date = moment().utc();  /* $scope.taskDBField.created_by = JSON.stringify({
                                 name: $localStorage.user_info.username,
                                 id: $localStorage.user_info.user_id
                             });*/
            }
            if ($scope.task_id) {
              Services.tasksName.update({ id: $stateParams.taskId }, $scope.taskDBField, function (data) {
                $scope.logger = {};
                $scope.logger.userid = $localStorage.user_info.user_id;
                $scope.logger.user_detail = JSON.stringify({
                  'username': $localStorage.user_info.username,
