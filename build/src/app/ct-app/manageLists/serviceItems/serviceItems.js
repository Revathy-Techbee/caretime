angular.module('ctApp.serviceItems', ['ui.router']).config([
  '$stateProvider',
  ...
  function ($scope, Services, $state, $modal, $localStorage) {
    ...
    $scope.enableEditView = function (id) {
      $state.go('ctApp.addUpdateServiceItem', { serviceItemId: id });
    };
    $scope.enableDelete = function (id) {
      ...
      $scope.modalInstance.result.then(function (id) {
        Services.service_item.delete({ filter: 'id=\'' + id + '\'' }, function (data) {
          $scope.logger = {};
  ...
  function ($scope, Services, $state, $stateParams, $timeout, HelperService, $localStorage) {
    ...
    $scope.getServiceItemDetail = function () {
      if ($scope.serviceItem_id) {
        ...
        Services.service_item.get({ filter: 'id=\'' + $scope.serviceItem_id + '\'' }, function (resp) {
      ...
      if ($scope.addUpdateserviceItemForm.$valid) {
        ...
        Services.service_item.get($scope.filterObj, function (data) {
          if (data.record.length > 0) {
            ...
            if ($scope.serviceItem_id) {
              Services.service_item.update({ id: $stateParams.serviceItemId }, $scope.serviceItemDBField, function (data) {
                $scope.logger = {};
                $scope.logger.userid = $localStorage.user_info.user_id;
                $scope.logger.user_detail = JSON.stringify({
                  'username': $localStorage.user_info.username,
